...
def _b9e55b6 : SortInt → SortWordStack → Option SortWordStack
  | 1, SortWordStack.«_:__EVM-TYPES_WordStack_Int_WordStack» _Gen0 WS => some WS
  | _, _ => none
...
def _6bcd2b6 : SortWordStack → SortInt → Option SortInt
  | SortWordStack.«_:__EVM-TYPES_WordStack_Int_WordStack» W _Gen0, N => do
...
def _bdd56b8 : SortWordStack → SortInt → Option SortInt
  | _Gen0, N => do
    let _Val0 <- «_<Int_» N 0
    guard _Val0
    return 0
...
def _ced9f91 : SortInt → SortWordStack → Option SortWordStack
  | N, SortWordStack.«.WordStack_EVM-TYPES_WordStack» => do
    let _Val0 <- «_>Int_» N 0
    guard _Val0
    return SortWordStack.«.WordStack_EVM-TYPES_WordStack»
  | _, _ => none
...
def _e40572a : SortInt → SortWordStack → Option SortWordStack
  | N, WS => do
    let _Val0 <- «_>Int_» N 0
    let _Val1 <- notBool_ _Val0
    guard _Val1
...
mutual
  partial def dropWordStack (x0 : SortInt) (x1 : SortWordStack) : Option SortWordStack := (_b9e55b6 x0 x1) <|> (_ced9f91 x0 x1) <|> (_e40572a x0 x1) <|> (_f4b4436 x0 x1)

  partial def _f4b4436 : SortInt → SortWordStack → Option SortWordStack
    | N, SortWordStack.«_:__EVM-TYPES_WordStack_Int_WordStack» W WS => do
      let _Val0 <- «_>Int_» N 1
      let _Val1 <- «_-Int_» N 1
      let _Val2 <- dropWordStack _Val1 (SortWordStack.«_:__EVM-TYPES_WordStack_Int_WordStack» W WS)
      let _Val3 <- dropWordStack 1 _Val2
      guard _Val0
      return _Val3
    | _, _ => none
end
...
mutual
  partial def «_[_]_EVM-TYPES_Int_WordStack_Int» (x0 : SortWordStack) (x1 : SortInt) : Option SortInt := (_6bcd2b6 x0 x1) <|> (_a6eb26a x0 x1) <|> (_bdd56b8 x0 x1)

  partial def _a6eb26a : SortWordStack → SortInt → Option SortInt
    | WS, N => do
      let _Val0 <- «_>Int_» N 0
      let _Val1 <- dropWordStack N WS
      let _Val2 <- «_[_]_EVM-TYPES_Int_WordStack_Int» _Val1 0
      guard _Val0
      return _Val2
end
