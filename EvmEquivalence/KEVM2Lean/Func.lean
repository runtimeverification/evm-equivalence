...
axiom «substrBytes(_,_,_)_BYTES-HOOKED_Bytes_Bytes_Int_Int» (x0 : SortBytes) (x1 : SortInt) (x2 : SortInt) : Option SortBytes
...
noncomputable def _ecc9011 : SortBytes → SortInt → SortInt → Option SortBytes
...
noncomputable def EVM_TYPES_bytesRange : SortBytes → SortInt → SortInt → Option SortBytes
  | WS, START, WIDTH => do
    let _Val0 <- «_>=Int_» WIDTH 0
    let _Val1 <- «_>=Int_» START 0
    let _Val2 <- _andBool_ _Val0 _Val1
...
noncomputable def «#range» (x0 : SortBytes) (x1 : SortInt) (x2 : SortInt) : Option SortBytes := (EVM_TYPES_bytesRange x0 x1 x2) <|> (_ecc9011 x0 x1 x2) <|> (_f005287 x0 x1 x2)

noncomputable def _ef5332a : SortBytes → Option SortInt
  | WS => do
...
noncomputable def asWord (x0 : SortBytes) : Option SortInt := _ef5332a x0
